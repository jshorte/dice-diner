...
func process_score(dice: Dice) -> void:
	...
	for entry in collision_log:
		...
		if other_dice._score_type == G_ENUM.ScoreType.FLAT:
			dice.set_flat_value(dice.get_flat_value() + other_dice.get_flat_value())
			other_dice.reported_score += other_dice.get_flat_value() * dice.get_base_quality_multiplier()
			...
			if other_dice._type == G_ENUM.DiceType.GARLIC:
				...
				update_multiplier_contribution(dice, other_dice)
